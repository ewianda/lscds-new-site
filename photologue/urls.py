...
urlpatterns = patterns('',
    ...
    url(r'^gallery/(?P<year>\d{4})/(?P<month>[-\w]+)/$',
    ...
    url(r'^gallery/$', cache_page(60 * 15)(GalleryArchiveIndexView.as_view()),
        name='pl-gallery-archive'),
    url(r'^$',
        RedirectView.as_view(url=reverse_lazy('pl-gallery-list',kwargs={'page':'1'})),
        name='pl-photologue-root'),
    url(r'^gallery/(?P<slug>[\-\d\w]+)/$', GalleryDetailView.as_view() , name='pl-gallery'),
    url(r'^gallery/page/(?P<page>[0-9]+)/$',cache_page(60*60*24)(GalleryListView.as_view()), name='pl-gallery-list'),
